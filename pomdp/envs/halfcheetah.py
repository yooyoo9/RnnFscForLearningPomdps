...
class HalfCheetahEnv(gym.ObservationWrapper):
    def __init__(self, pomdp_type='remove_velocity',
    ...
    def _remove_velocity():
        ...
        obs_low = np.array([-np.inf for _ in range(len(remain_obs_idx))], dtype="float32")
        obs_high = np.array([np.inf for _ in range(len(remain_obs_idx))], dtype="float32")
