...
class FscActorCritic(ActorCritic):
    def __init__(
        ...
        self.o_buff_len = 0
        super(FscActorCritic, self).__init__(
            env,
            gamma,
            seed,
            actor_lr,
    ...
    def step(self, o):
        h_o = (
            torch.FloatTensor(self.o_buff)
            .view(1, self.o_buff.shape[0], self.o_buff.shape[1])
            .to(self.device)
        )
        h_a = (
            torch.FloatTensor(self.a_buff)
            .view(1, self.a_buff.shape[0], self.a_buff.shape[1])
            .to(self.device)
        )
